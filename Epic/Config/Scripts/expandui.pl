...
############################## Start Script ###################################
###### Written by General BlackDragon and Ken Miller.
...
while($line = <STDIN>)
{
	...
	elsif($line =~ /^\s*Cmd\(/i)
	{
		...
		else
		{
			...
			$indexopen += 2;
			my $lineLeft = substr($line, 0, $indexopen); # up to ("
			my $lineMid = substr($line, $indexopen, $indexclose - $indexopen); # between (""), i.e. 'iface.shellresx 640'
			my $lineRight = substr($line, $indexclose, 999999); # ") to end of line

			my @myarray = split(' ', $lineMid);
			if ($myarray[0] =~ "iface.shellresx")
			{
				$myarray[1] = floor(($myarray[1] * $xScaleMultiplier) + 0.5);
			}
			elsif ($myarray[0] =~ "iface.shellresy")
			{
				$myarray[1] = floor(($myarray[1] * $yScaleMultiplier) + 0.5);
			}
